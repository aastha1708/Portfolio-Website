 ...
 *
 * Two states, one layout: a written-up project is a link that lifts and zooms
 * under the pointer, an unwritten one (`comingSoon`) is inert and says so
 * through the cursor caption. The difference is deliberately only in the
 * behaviour, never in the styling — the work deserves the same presentation
 * either way, and a visually downgraded card would read as weaker work rather
 * than as an unfinished write-up.
 */
export default function ProjectCard({ project, fixed = true }: { project: Project; fixed?: boolean }) {
    ...
    >
      {project.video ? (
        <>
          ...
          <video
            className={`absolute inset-0 size-full object-cover transition-transform duration-[600ms] ease-[cubic-bezier(0.22,1,0.36,1)] motion-reduce:hidden motion-reduce:transition-none ${
        ...
        <Image
          ...
          fill
          sizes="(max-width: 1024px) 100vw, 502px"
          className={`object-cover transition-transform duration-[600ms] ease-[cubic-bezier(0.22,1,0.36,1)] motion-reduce:transition-none ${
            live ? "group-hover:scale-[1.04]" : ""
          }`}
        />
      )}
  ...
  if (!live) {
    return (
      /* Not a link and not focusable: there is nowhere to go yet. The caption
         carries the state for pointer users; the visually-hidden note carries
         the same thing for everyone else. */
      <div className="block" data-cursor="label" data-cursor-text="Coming soon">
        {card}
