 ...
 *
 * Each `box` is the object's visual footprint on the 1440 canvas. These were
 * cross-checked against the source PNGs: every box's aspect ratio matches its
 * asset to within a rounding error, so each file is placed directly with
 * object-contain rather than reproducing Figma's internal fill crops (which
 * describe a different, composited source and don't transfer).
 *
 * `rotate` is only set where Figma's rotation is genuinely visible in the
 * render — elsewhere the fill transform cancels it out and net rotation is 0.
 *
...
export const HERO_ITEMS: CollageItem[] = [
  ...
  /* --- tool logos: real outbound links --- */
  {
    id: "antigravity",
